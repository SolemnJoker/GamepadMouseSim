...
QVector<HelpSection> build(const Config& config) {
    ...
    sections.push_back({Translations::tr("直接映射"),
                        mappingSection(direct, QStringList{"A", "B", "X", "Y", "LB", "RB"})});
    sections.push_back(
        {Translations::tr("方向键(直接)"),
         mappingSection(direct, QStringList{"DpadUp", "DpadDown", "DpadLeft", "DpadRight"})});
    sections.push_back({Translations::tr("L3层 (按住L3)"), mappingSection(lt, buttonOrder)});
    sections.push_back({Translations::tr("RT层 (按住RT)"), mappingSection(rt, buttonOrder)});

    // 摇杆区:参数来自配置,文案固定。
    QVector<HelpLine> stick;
    stick.push_back({QStringLiteral("左摇杆"), Translations::tr("移动鼠标")});
    stick.push_back({QStringLiteral("右摇杆"), Translations::tr("滚动页面")});
    sections.push_back({Translations::tr("摇杆"), stick});
    ...
    if (!comboBtns.isEmpty())
        system.push_back(
            {comboBtns.join("+") + Translations::tr("(长按1秒)"), Translations::tr("切换模式")});
    ...
    sections.push_back({Translations::tr("模式与系统"), system});
