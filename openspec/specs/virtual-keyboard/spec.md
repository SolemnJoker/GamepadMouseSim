...
系统 SHALL 提供"切换虚拟键盘"按钮动作(`ShowKeyboard`),用于显示/隐藏虚拟键盘 overlay。默认绑定 SHALL 为 LT 修饰层 + Menu 键(物理手势 = 按住 L3 + 按 Menu);该动作 SHALL 作为一等 `ButtonAction` 出现在按键映射体系中,并因此可通过现有配置机制自定义绑定。
